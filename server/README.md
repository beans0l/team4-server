...
| 다운 | text | `{"type":"safety_blocked","reason":"..."}` — 이 턴은 막혔다. **앱은 무시한다**(아래) |
| 다운 | text | `{"type":"turn_complete"}` — 한 턴 끝 |
| 다운 | text | `{"type":"session_reset"}` — 재연결됨. **이전 대화 맥락이 사라졌다** |
| 다운 | text | `{"type":"error","code":"...","message":"..."}` |

**`safety_blocked` — 앱은 구현하지 않아도 된다 (2026-08-20 결정)**

유해 콘텐츠 필터에 걸린 턴은 Gemini 가 **오디오를 한 조각도 보내지 않는다.** 그래서 이
프레임을 받은 턴에는 인형이 침묵한다. **그대로 두기로 했다** — 앱의 `TalkSocket` 은 모르는
프레임을 로그만 남기고 무시하므로 **추가 구현이 필요 없다.**

근거: 도발 5종·대조군 2종 실측에서 **차단이 한 번도 일어나지 않았다**(0/7). Gemini 내장
필터가 그만큼 느슨해서, 폴백을 만들어도 거의 실행되지 않는 코드가 된다. 아동 안전은
페르소나의 `안전:` 절이 맡는다(같은 실측에서 7/7 적절히 대응).

> 서버는 이 프레임을 계속 보낸다. 지우지 않은 이유는 `Usage.blocked_turns` 가
> **차단이 실제로 나는지 아는 유일한 수단**이기 때문이다. 이게 없으면 시연에서 인형이
> 침묵했을 때 원인이 차단인지 네트워크인지 구분할 방법이 없다.

폴백을 나중에 붙이려면 (지금은 불필요):
- 대사 wav 생성은 `.venv/bin/python -m ai.dialog_test --fallback` (산출물 `ai/out/fallback/`)
- 🔴 **`DollSpeaker` 의 `AudioTrack` 경로로 밀어 넣어야 한다.** 별도 `MediaPlayer` 로
  재생하면 폴백이 나오는 동안 **입이 움직이지 않는다**(립싱크가 RMS 기반이라).
- ⚠️ voice 가 `DOLL_VOICE` 와 같아야 한다. 다르면 딴 인형이 끼어든 것처럼 들린다.

`reason` 은 디버깅·집계용이다. 항상 `turn_complete` **직전**에 온다.
